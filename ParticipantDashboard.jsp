    ...
    <div class="main-container">
      <div class="sidebar">
        ...
        <div class="sidebar-nav">
          <ul>
            ...
            <li><a role="button" onclick="showFeedback()" href="#">Feedback</a></li>
          </ul>
        </div>
      </div>
      <div class="dashboard-container">
        <div class="dashboard">
            ...
            <% ArrayList ar = (ArrayList)request.getAttribute("ar"); 
            	...
            	for(int i=0 ; i<ar.size() ; i++){
	              ...
	              <tr>
	                ...
	                <td><%= pt.getStart() %></td>
	                <td><%= pt.getEnd() %></td>
	                <td class="active" ><span><%= pt.getStatus() %></span></td>
	                <td><a style="color :blue;" href="submission.jsp?cid=<%= pt.getCid()%>&cname=<%= pt.getCname()%>">Add Sub</a></td>
	              </tr>
              <%} %>
            </tbody>
          </table>
		  ...
		  <!-- table to completed Competitions list -->
          ...
          <table cellspacing="5px">
            ...
            <tbody>
            <% ArrayList cr = (ArrayList)request.getAttribute("cr"); 
            	
            	for(int i=0 ; i<cr.size() ; i++){
            		
            		Participant pt = (Participant)cr.get(i);
	              ...
	              <tr>
	               ...
	               <% String pq = "select points from result where pid = ? and cid = ?";
	               	  
	               	try{
	               		Class.forName("com.mysql.cj.jdbc.Driver");
	                  	Connection c = DriverManager.getConnection("jdbc:mysql://localhost:3306/competative_world","root","1234");
	               	  	
	                  	PreparedStatement smt = c.prepareStatement(pq);
	                  	
	                  	smt.setString(1,(String)session.getAttribute("email"));
	                  	smt.setInt(2,pt.getCid());
	               	  
	                  	ResultSet rs = smt.executeQuery();
	               
	               		if(rs.next()){
	               %>
	                <td><a style="color :blue;" href=""><%= rs.getFloat(1) %></a></td>
	              </tr>
	              <%} else { %>
            ...
            <% 
            	for(int i=0 ; i<cr.size() ; i++){
	              ...
	              <tr>
	                ...
	                <td><%= pt.getEnd() %></td>
	                <td class="active" ><span><%= pt.getStatus() %></span></td>
	                <td><a style="color :blue;" href="feedback.jsp?cid=<%= pt.getCid()%>&cname=<%= pt.getCname()%>">Give Feedback</a></td>
	              </tr>
              <%} %>
